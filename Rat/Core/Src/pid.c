...
float angleCorrection2;
float distanceCorrection2;
float kPw = 0.05;
float kDw = 0.06;
float kPx = 0.05;
float kDx = 0;
...
void updatePID() {
		...
		angleCorrection2 = angleCorrection;
		distanceCorrection2 = distanceCorrection;

		if (distanceCorrection > 0.5 && distanceCorrection > 0){
			distanceCorrection = 0.5;
		}
		else if (distanceCorrection < -0.5 && distanceCorrection < 0 ){
			distanceCorrection = -0.5;
		}
		if (angleCorrection > 0.3 && angleCorrection > 0){
			angleCorrection = 0.3;
		}
		else if(angleCorrection < -0.3 && angleCorrection < 0){
			angleCorrection = -0.3;
		}
		...
		if (angleCorrection < 0){
			...
			setMotorLPWM ( distanceCorrection + angleCorrection + smallAccount);
			setMotorRPWM (distanceCorrection - angleCorrection + smallAccount );

		if ( (distanceError < 10 && distanceError > -10) && (angleError < 5 && angleError > -5)){
			done += 1;
		}
		else{
			done = 0;
